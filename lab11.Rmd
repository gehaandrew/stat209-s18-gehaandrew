...
Answer: As the average mass of male birds increases, so too does the average mass of its eggs.
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  annotate("text", x = 350, y = 5, label = "Red Points = Parrots") +
  scale_x_log10() +
  scale_y_log10()
...
Answer: Yes, the bet-fit line matches the pattern that as the size of the male bird increases, so does the size of its eggs.
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  scale_y_log10() +
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5) +
  annotate("point", x = 218.7, y = 87.00, color = "firebrick2", size = 3) +
  annotate("text", x = 218.7, y = 115, label = "Red-Tailed Tropicbird", size = 3)
...
```{r}
albatross <- filter(birds, type %in% c("Albatross"))
albatross
```

```{r}
warbler <- filter(birds, type %in% c("Warbler"))
warbler
```
...
ggplot(birds, aes(male_mass, clutch_size)) +
  geom_point(alpha = 0.15) +
  geom_point(color = "indianred2", data = albatross) +
  geom_point(color = "blue3", data = warbler) +
  scale_x_log10() +
  scale_y_log10() +
   geom_smooth(method = "lm", color = "deeppink", se = FALSE, linetype = "dashed", size = 0.5 ) +
  annotate("text", x = 5000, y = 1.1, label = "Red Points = Albatross", size = 3) +
  annotate("text", x = 5, y = 7, label = "Green Points = Warbler", size = 3) +
  xlab("Average Male Mass") +
  ylab("Clutch Size") +
  ggtitle("Clutch Size Based on Average Male Mass")
